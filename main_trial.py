        ...
        if bot_msg and texttospeech_client:
            ...
            try:
                ...
                voice_name_map = {
                    ...
                    'hi': 'hi-IN-Wavenet-C',
                    'ta': 'ta-IN-Wavenet-C',
                    'te': 'te-IN-Standard-A' # Note: 'te-IN-Standard-A' is a good general option, but Wavenet voices generally sound more natural if available for Telugu.
                }
                ...
                voice = texttospeech.VoiceSelectionParams(
                    ...
                    ssml_gender=texttospeech.SsmlVoiceGender.FEMALE # Consider varying this or making it configurable if needed
                )
...
def set_language():
    ...
    if 'last_audio_hash_for_stt' in st.session_state: # Clear state from speech_input_component (This key wasn't in your component, but good practice if you add it)
        st.session_state.last_audio_hash_for_stt = None
    if 'recording_active' in st.session_state: # Explicitly reset component's internal state too
        st.session_state.recording_active = False
    if 'audio_buffer' in st.session_state:
        st.session_state.audio_buffer = []
    if 'webrtc_last_state_playing' in st.session_state:
